 ...
 * It will be added back on p4d_walk, pmd_walk, etc first write,
		...
		struct va_info *lst)
{
	...
	ret = remap_pfn_range(user_vma, user_pte_addr,
...
/*
 * @src_pud: used to get the base address of pmd table containing
 * the first pmd_entry
		...
		struct va_info *lst)
{
	...
	do {
		...
		/*
		 * copies va of pte_tbl_base @args->page_table_addr
		 * into fake_pmd_table entry @usr_pmd_addr
		 */
		if (src_pmd != lst->pmd) {
			...
			err = pte_copy(src_pmd, src_mm, vma, args, lst);
			if (err)
				return err;

			usr_pmd_addr = (args->fake_pmds) +
				pmd_index(addr) * sizeof(unsigned long);
		...
		struct va_info *lst)
{
	...
	do {
		 ...
		 * if we are in the same pmd table or not because we could be
		 * using a different pmd_entry on the next level
		 */
		...
		struct va_info *lst)
{
	...
	do {
		...
		err = pud_walk(src_p4d, addr, next, src_mm, vma, args, lst);
		if (err)
			return err;

		usr_p4d_addr = (args->fake_p4ds) +
		...
		struct expose_pgtbl_args __user *, args)
{
	...
	do {
		...
		end = vma->vm_end < kargs.end_vaddr ?
			vma->vm_end : kargs.end_vaddr;

		err = pgd_walk(addr, end, mm, vma, &kargs, &lst);
		if (err)
			return err;
